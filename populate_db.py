        ...
        try:
            ...
            for (user_name, club_name, club_id) in res:
                ...
                for j in range(0, random.randint(1, 5)):
                    cursor.execute(
                        "INSERT INTO post (user_id, club_id, date, likes, title, content) VALUES (%s, %s, %s, %s, %s, %s)",
                        (
                            i,
                            club_id,
                            datetime.datetime.utcnow() - datetime.timedelta(hours=random.randint(1, 6),
                                                                            minutes=random.randint(0, 60)),
                            random.randint(0, 17),
                            "USER: " + user_name + ", title!",
                            "Round 2 post in " + club_name + " this is my content ROUND 2, in club: " + club_name+", Random repetition: "+ str(j))
    ...
    for i in range(1, 18):
        try:
            ...
            for (user_name, club_name, post_id) in res:
                cursor.execute(
                    "INSERT INTO comments (user_id, post_id, date, likes,  content) VALUES (%s, %s, %s, %s, %s)",
                    (
                        i,
                        post_id,
                        datetime.datetime.utcnow() - datetime.timedelta(hours=random.randint(1, 2),
                                                                        minutes=random.randint(0, 60)),
                        random.randint(0, 17),
